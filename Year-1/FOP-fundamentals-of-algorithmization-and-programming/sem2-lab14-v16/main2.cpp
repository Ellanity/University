...
public:
	...
	vector<leaf*>  get_leaves();
	int   number_of_leaves(int n = 0);
	int   tree_depth(int level = 0);

	friend bool compareLeavesById(leaf* leaf1, leaf* leaf2);
};

bool compareLeavesById(leaf* leaf1, leaf* leaf2)
{
...
bool compareLeavesById(leaf* leaf1, leaf* leaf2)
{
	return (leaf1->get_id() < leaf2->get_id());
}
*/
...
int main() {
	...
	do
	{
		...
		if (choice == 9) {
			vector < leaf* > leaves = root->get_leaves();
			sort(leaves.begin(), leaves.end(), compareLeavesById);
			for (auto leaf : leaves) {
				cout << leaf->get_id() << " " << leaf->get_information() << "\n";
			}
		}
		if (choice == 10) {
			choice == 0;
...
vector<leaf*> leaf::get_leaves()
{
	vector<leaf*> leaves, left_leaves, right_leaves;
	int n = 0;
	if (this->left == NULL && this->right == NULL)
		leaves.push_back(this);
	if (this->left != NULL)
		left_leaves = this->left->get_leaves();
	if (this->right != NULL)
		right_leaves = this->right->get_leaves();
	for (leaf* leaf : left_leaves)
		leaves.push_back(leaf);
	for (leaf* leaf : right_leaves)
		leaves.push_back(leaf);
	return leaves;
}
