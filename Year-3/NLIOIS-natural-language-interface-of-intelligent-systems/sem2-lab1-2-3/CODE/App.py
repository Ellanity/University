...
class Application(Frame):
    ...
    def __initUI__(self):
        ...
        # parse text button
        self.parse_button = ButtonCustom(self.content, "Parse text", command=lambda: self.parse_text())
        self.parse_button.pack()

        # parse words button
        self.parse_button = ButtonCustom(self.content, "Parse words", command=lambda: self.parse_words())
        self.parse_button.pack()
    ...
    def parse_text(self):
        self.last_state = "parse"
        input_text = self.input_text.get(1.0, END)
        output_text = ""
        trans_table = {ord('\n'): None, ord('\t'): None, ord('.'): ".\n"}
        sentences = input_text.translate(trans_table).split('.')
        for sentence in sentences:
            processed = self.parser.syntax_analysis_component_systems(sentence)
            output_text += str(processed)

        self.output_text.delete(1.0, END)
        self.output_text.insert(END, output_text)

        for sentence in sentences:
            self.parser.syntax_analysis_subordination_trees(sentence)
        print(self.parser.subordination_trees_filled)

    def parse_words(self):
        self.last_state = "parse"
        input_text = self.input_text.get(1.0, END)
        trans_table = {ord('\n'): None, ord('\t'): None, ord('.'): ".\n"}
        input_text = input_text.translate(trans_table)
        processed = self.parser.parse_words(input_text)
        #output_text = ""
    ...
    def load_file(self):
        ...
        if filename:
            with open(filename, 'rb') as file:
                ...
                text = text.translate(file_trans_table)
